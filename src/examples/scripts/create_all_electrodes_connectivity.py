...
def run(mmvt):
    ...
    create_all_connections(mmvt, parent, conn_parent_obj, orig_curve, layers_rods)
    bpy.ops.object.select_all(action='DESELECT')
    bpy.data.objects[orig_curve.name].select = True
    bpy.ops.object.delete()


def create_all_connections(mmvt, parent, conn_parent_obj, orig_curve, layers_rods):
    mu = mmvt.utils
    ...
    children = [bpy.data.objects[elc_name] for elc_name in [
        'LOF11-LOF10', 'LAF8-LAF7', 'LAT5-LAT4', 'RPF2-RPF1', 'RPF7-RPF6', 'LOF4-LOF3', 'LAT3-LAT2']]
    for ind1, elc_obj1 in enumerate(children):
        for ind2, elc_obj2 in enumerate(children):
            if ind1 == ind2:
                continue
            ...
            con[ind1, ind2] = True
            cur_obj = copy_curve(
                orig_curve, elc_obj1, elc_obj2, conn_parent_obj.name, layers_rods, bevel_depth=0.1, resolution_u=1)
