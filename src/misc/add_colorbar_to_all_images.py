...
def add_colorbar(subject, data_max=1, data_min=0.95, colors_map='YlOrRd', file_type='jpeg'):
    figures = glob.glob(op.join(MMVT_DIR, subject, 'figures', '**', '*.{}'.format(file_type)), recursive=True)
    for figure_fname in tqdm(figures):
    ...
    color_bars = glob.glob(op.join(MMVT_DIR, subject, 'figures', '**', 'YlOrRd_colorbar.jpg'), recursive=True)
